    ...
    createQuery(tableName = this.currentTable) {
        ...
        let obj = {
            ...
            async getPageAndCount() {
                // 第一步:拼接SQL
                let sql1 = this.listSql + (this.innerJoinFields.length > 0 ? "," : "") + this.innerJoinFields.join(",") + this.from + this.innerJoinStr + this.where + this.strWhere + this.orderByStr + ` limit ${(this.pageIndex - 1) * 5} , 5 `;
